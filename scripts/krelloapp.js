...
// -----------------------------
// Хранение HTML колоночных досок
// -----------------------------
const boardData = {
    inbox: null,
    boards: {
...
function saveCurrentBoard() {
    ...
    if (ui.currentSection === 'inbox') {
        ...
        boardData.boards[ui.currentBoard] = html;
    }
    ...
    if (section === 'inbox') {
        ...
        if (boardData.boards[board]) {
            ...
            boardColumns.innerHTML = `
                <div class="column" data-column-id="${ui.columnIdCounter++}">
    ...
    menu.addEventListener('click', (e) => {
        ...
        switch (action) {
            ...
            case 'copy-list': {
                ...
                copy.setAttribute('data-column-id', ui.columnIdCounter++);
...
// -----------------------------
// Sections (Inbox | Boards | Favorites)
// -----------------------------
...
function renderFavorites() {
    ...
    if (favorites.length === 0) {
        boardColumns.innerHTML = `
            <div style="color:white;padding:20px;">
                ...
                <br>
                <small>Нажмите на звездочку, чтобы добавить</small>
            </div>`;
        return;
    }
    ...
    if (boardData.inbox) {
        ...
        all.push(...doc.querySelectorAll('.card'));
    }

    Object.values(boardData.boards).forEach(html => {
        if (html) {
    ...
    const container = wrap.querySelector('.column-cards');
...
function filterCards(text) {
    ...
    if (!text) {
        ...
        document.querySelectorAll('.column').forEach(col => col.classList.remove('hidden'));
        return;
    }
...
function createNewColumn(name) {
    if (ui.currentSection === 'favorites') {
        alert('Нельзя создавать колонны в избранном');
        return;
    }
    ...
    col.dataset.columnId = ui.columnIdCounter++;
...
function addNewCard(column) {
    ...
    card.className = 'card';
    card.dataset.cardId = String(ui.cardIdCounter++);
    ...
    document.querySelectorAll('.card').forEach(card => {
        card.onclick = (e) => {
            ...
            if (edited && edited.trim() && edited !== text) {
                card.querySelector('p').textContent = edited.trim();
                saveCurrentBoard();
            }
        };
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!boardData.inbox) saveCurrentBoard();
